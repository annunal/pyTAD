...
#  
#  objective:  to realize a command listener on Teltonika remote device based on SMS
#
# how to install
#
...
if [ "$1" = "config" ];then
	...
	ip=$(ifconfig eth0| sed -En 's/127.0.0.1//;s/.*inet (addr:192)?(([0-9]*\.){3}[0-9]*).*/\2/p')

	echo 'my IP is:' $ip
	echo $ip>sms_IP.txt
	echo 'copying config.txt on Teltonika'
	sshpass -p "Ecml2011" scp sms_config.txt root@192.168.1.1:/sbin/sms_config.txt
	echo 'copying ip.txt on Teltonika'
	sshpass -p "Ecml2011" scp sms_IP.txt root@192.168.1.1:/sbin/sms_IP.txt
	echo 'copying the script on Teltonika'
	sshpass -p "Ecml2011" scp sms_command.sh root@192.168.1.1:/sbin/sms_command.sh
	...
	sshpass -p "Ecml2011" scp sms_init.sh root@192.168.1.1:sms_init.sh
	sshpass -p "Ecml2011" ssh root@192.168.1.1 'chmod +x sms_init.sh;sh sms_init.sh'
	cd /home/pi
